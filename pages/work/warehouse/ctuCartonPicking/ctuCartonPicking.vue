					...
					<view class="header-content">
						...
						<view class="right">
							<view class="action-btn" @click="createLabel">
								...
								<text>{{$t('warehouse.genCartonCode')}}</text>
							</view>
						</view>
					</view>
				</view>
		...
		<uni-popup ref="pickingDrawer" type="bottom" :safe-area="false">
			<view class="drawer-content">
				...
				<scroll-view scroll-y class="drawer-body">
					...
					<view class="form-item">
						<text class="form-label">{{$t('warehouse.worksheetCode')}}</text>
						<input class="form-input" v-model="formData.worksheetCode" :placeholder="$t('warehouse.worksheetCode')" />
					</view>
					<view class="form-item">
						<text class="form-label">{{$t('warehouse.classType')}}</text>
						<input class="form-input" v-model="formData.classType" :placeholder="$t('warehouse.classType')" />
					</view>
					<view class="form-item">
						<text class="form-label">{{$t('warehouse.lineType')}}</text>
						<input class="form-input" v-model="formData.lineType" :placeholder="$t('warehouse.lineType')" />
					</view>
					<view class="form-item">
						<text class="form-label">{{$t('warehouse.batchCode')}}</text>
						<input class="form-input" v-model="formData.batchCode" :placeholder="$t('warehouse.batchCode')" />
					</view>
				</scroll-view>
			...
			createLabel: async function() {
				try {
					...
					if (resp.code == 200) {
						...
						this.reloadTable();
						showBeautyToast({
							title: this.$t('warehouse.cartonGenSuccess'),
							icon: 'success'
						})
					} else {
						showBeautyToast({
