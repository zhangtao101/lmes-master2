			...
			<view class="task-card" v-for="(task, index) in filteredTaskList" :key="index" @click="onTaskClick(task)">
				<view class="task-header">
					<view class="task-title-wrapper">
						<text class="task-title">{{ task.taskTypeName || $t('warehouse.maintainTask') }}</text>
					</view>
				...
				<view class="task-footer">
					...
					<text>{{ currentStatus === 'PENDING' ? $t('warehouse.enterMaintain') : $t('warehouse.viewDetail') }}</text>
				</view>
			</view>
		</view>
		...
		methods: {
			...
			onScanCode() {
				...
				scanCode().then((code) => {
					...
					showBeautyToast({
						title: err || _this.$t('warehouse.scanFail'),
						icon: 'none'
					});
				});
			},
