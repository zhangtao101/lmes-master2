<template>
	<view class="wsq-wrapper">
		<!-- 底部弹层 -->
		<uv-popup ref="wsqPopup" mode="bottom" bgColor="#f5f7fa" :round="24" :closeOnClickOverlay="false"
			custom-style="height: 90vh; padding: 24rpx; box-sizing: border-box;">
			<view class="wsq-body">
				<!-- 标题 -->
				<view class="wsq-title">{{$t('sanyan.wsqTitle')}}</view>

				<!-- 1. 查询条件：日期范围 + 线别（线别由父组件传入，仅展示不参与实际查询） -->
				<view class="wsq-card">
					<uv-form :model="form" :border-bottom="false" :label-width="110" label-position="left">
						...
						<uv-form-item :label="$t('sanyan.lineLabel')">
							<view class="line-view">
								<text :class="lineName ? 'value' : 'placeholder'">
									{{ lineName || $t('sanyan.lineNotSelected') }}
								</text>
							</view>
						</uv-form-item>
					</uv-form>
					<view class="wsq-actions">
						<view class="wsq-btn" @click="onReset">{{$t('common.reset')}}</view>
						<view class="wsq-btn primary" :class="{ disabled: loading }" @click="onQuery">
							{{$t('sanyan.query')}}
						</view>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			open: function() {
				...
				this.$refs.wsqPopup && this.$refs.wsqPopup.open('bottom');
				this.onQuery();
			},
			// 关闭弹层
			close: function() {
				this.$refs.wsqPopup && this.$refs.wsqPopup.close();
			},
			...
			// 拉取工单列表（按子产线ID + 指示日期区间条件查询，固定正序 isAsc = 1）
			fetchList: function() {
				const _this = this;
				if (_this.loading) return;
				// 未传入子产线ID时不发请求，避免无过滤条件拉到全部工单
				if (!_this.lineId) {
					_this.rows = [];
					_this.total = 0;
					_this.pageCount = 1;
					uni.showToast({
						title: _this.$t('sanyan.lineNotSelected'),
						icon: 'none'
					});
					return;
				}
				...
				// timeRange 数组格式化为 planDateStart / planDateEnd
				if (_this.form.timeRange && _this.form.timeRange.length === 2) {
					params.planDateStart = _this.form.timeRange[0];
					params.planDateEnd = _this.form.timeRange[1];
				}
				vegetablePacking.searchWorksheet(params).then(function(res) {
					_this.loading = false;
					if (res.code == 200) {
						const data = res.data || {};
						// 接口约定返回 { total, count, results }，同时兼容直接返回数组
						const list = Array.isArray(data) ? data : (data.results || data.list || []);
						_this.rows = list;
						_this.total = Number(data.total) || list.length;
						// 总页数按总条数计算；总条数缺失时用「本页是否满页」兜底
						if (_this.total > 0) {
							_this.pageCount = Math.max(1, Math.ceil(_this.total / _this.page.pageSize));
						} else {
							_this.pageCount = list.length >= _this.page.pageSize ? _this.page.pageNum + 1 : 1;
						}
						// 末页可能返回空列表，回退页码防止越界
