<template>
	<view class="warehouse-query-page">
		<!-- 1. 查询条件 -->
			...
			<uv-form :model="form" :border-bottom="false" :label-width="110" label-position="left">
				<uv-form-item :label="$t('sanyan.materialCodeLabel')">
					<view class="input-row">
						<uv-input v-model="form.materialCode" :placeholder="$t('sanyan.materialCodePlaceholder')"
							:clearable="true" confirm-type="search"
							custom-style="flex: 1; height: 38px; box-sizing: border-box;"
							@confirm="onMaterialCodeConfirm" />
						<view class="scan-btn" @click="onScan('materialCode')">
				...
				<uv-form-item :label="$t('sanyan.warehouseLabel')">
					<view class="input-row">
						<uv-input v-model="form.warehouseName" :placeholder="$t('sanyan.warehousePlaceholder')"
							:clearable="true" confirm-type="search"
							custom-style="flex: 1; height: 38px; box-sizing: border-box;"
							@confirm="onWarehouseConfirm" />
						<view class="scan-btn" @click="onScan('warehouseName')">
							<uni-icons type="scan" color="#fff" size="16"></uni-icons>
							<text>{{$t('sanyan.scan')}}</text>
						</view>
					</view>
		...
		<view class="list-card">
			<template v-if="rows.length">
				<view class="table-row table-header">
					<view class="table-cell" style="flex: 1.4;">{{$t('sanyan.warehouseLabel')}}</view>
					<view class="table-cell" style="flex: 1.4;">{{$t('sanyan.materialCodeLabel')}}</view>
					<view class="table-cell" style="flex: 2;">{{$t('sanyan.materialNameLabel')}}</view>
				</view>
				<scroll-view scroll-y class="table-body">
					<view class="table-row table-data" v-for="(row, index) in rows" :key="row.materialCode || index">
						<view class="table-cell highlight" style="flex: 1.4;">{{row.warehouseName || '--'}}</view>
						<view class="table-cell" style="flex: 1.4;">{{row.materialCode || '--'}}</view>
...
<script>
	import warehouseQueryApi from '/api/sanyang/weighPalletLoad.js';
	...
	export default {
		...
		methods: {
			// 品号回车确认：扫码枪回车极快，延迟 200ms 再取值，避免拿到不完整数据
			onMaterialCodeConfirm: function() {
				const _this = this;
				setTimeout(function() {
					if (_this.form.materialCode) {
						_this.onQuery();
					}
				}, 200);
			},
			// 仓库回车确认：同上，延迟 200ms 再取值
			onWarehouseConfirm: function() {
				const _this = this;
				setTimeout(function() {
					if (_this.form.warehouseName) {
						_this.onQuery();
					}
				}, 200);
			},
			// 扫码：field 为写入的字段名（materialCode / warehouseName）
			...
			fetchList: function() {
				...
				_this.loading = true;
				const params = {
					pageNum: String(_this.page.pageNum),
					pageSize: String(_this.page.pageSize)
				};
				if (_this.form.materialCode) params.materialCode = _this.form.materialCode.trim();
				if (_this.form.warehouseName) params.warehouseName = _this.form.warehouseName.trim();
				warehouseQueryApi.selectAllMaterial(params).then(function(res) {
					_this.loading = false;
					if (res.code == 200) {
						const data = res.data || {};
