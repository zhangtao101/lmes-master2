<template>
	<view class="ware-house-container">
		<view class="box-header">
			...
			<text class="title">{{ $t('warehouse.diaoboTitle') }}</text>
		</view>
		<view class="box-body">
					...
					<view class="label-item">
						<view class="left">
							...
							<uni-number-box :max="labelInfo.packageNumber"
								v-model="diaoboCount[`dbcount_${labelInfo.labelCode}`]"></uni-number-box>
						</view>
						<view class="right">
		...
		setTimeout(function() {
			...
			onSubmit: async function() {
				...
				this.labelList.forEach(label => {
					if (_this.diaoboCount[`dbcount_${label.labelCode}`] <= 0) {
						uni.showToast({
							title: _this.$t('warehouse.diaoboQtyTip')
						})
						canSubmit = false;
						return;
					}
					_labelList.push({
						...
						number: _this.diaoboCount[`dbcount_${label.labelCode}`],
						wareLocationCode: _this.wareLocationCode,
						logicalWarehouseCode: logicalCode,
					})
				})
				if (canSubmit) {
					const res = await material.transfer(_labelList);
					if (res.code == 200) {
						uni.showToast({
