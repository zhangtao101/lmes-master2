<template>
	<view class="ware-house-container">
		<view class="box-header">
			...
			<text class="title">{{ $t('warehouse.cpDiaoboTitle') }}</text>
		</view>
		<view class="box-body">
			...
			<view class="warehouse-detail">
				<view class="header">
					<view class="left">
						...
						<text class="title">{{ $t('warehouse.diaoboDetail') }}</text>
					</view>
				</view>
			<view class="header">
				<view class="left">
			...
			<view class="scan-input-row">
				...
				<view class="label-info-container" v-for="labelInfo in labelList" :key="labelInfo.labelCode">
					...
					<!-- <view class="label-item">
						<view class="left">
							<text>{{ $t('warehouse.diaoboQty') }}</text>
							<uni-number-box :max="+labelInfo.number"
								v-model="dbCount[`dbcount_${labelInfo.labelCode}`]"></uni-number-box>
						</view>
					</view> -->
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			loadWarehouse: async function(code) {
			...
			onSubmit: async function() {
				...
				this.labelList.forEach(label => {
					_labelList.push({
						...
						inLocationCode: _this.wareLocationCode,
						logicalInHouseCode: logicalCode
					})
				})

				const res = await product.saveTransfer(_labelList);
				if (res.code == 200) {
					uni.showToast({
						title: _this.$t('warehouse.diaoboSuccess'),
						icon: 'none'
					})
