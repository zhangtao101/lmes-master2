...
<script>
	...
	export default {
		data() {
			return {
				currentAssetType: 'EQUIP', // 'EQUIP' 设备, 'MOLD' 模具
				currentStatus: 'PENDING', // 'PENDING' 待执行, 'DONE' 已完成
				taskList: [],
				searchKey: '',
				loading: false,
				_searchTimer: null
		...
		methods: {
			...
			loadTaskList() {
				...
				equipmentSpotCheckApi.getInspectionTaskList(params).then(resp => {
					this.loading = false
			...
			onTaskClick(task) {
				uni.navigateTo({
					url: `/pages/work/warehouse/equipmentSpotCheck/equipmentSpotCheckDetail?taskId=${task.taskId}`
				})
			}
		}
	}
